...
export async function getProductsData() {
  const query = `query productPriceRanges @inContext(country: IN language: NL) {
    products(first: 250) {
      edges {
       node {
              id
...
export async function getProductByHandle(handle) {
  const query = `query productPriceRanges @inContext(country: ${"US"} language: NL){
...
export const getCollectionsByHandle = async (collection)=> {
  console.log("collection", collection);
  const query = `query productPriceRanges @inContext(country: ${"IN"}){
...
export async function FilterProducts(filter, sort,lang) {
  console.log("filter", filter, sort,lang);
  const query = `query productPriceRanges @inContext( country: ${"US"} language: NL){
